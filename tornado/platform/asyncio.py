...
class BaseAsyncIOLoop(IOLoop):
    '''
    loop 实现类，完成套接字的 读写 事件注册 和启动 loop
    '''
    ...
    # 事件注册操作，epoll select 操作最重要的事件监听机制
    # 其大意是 注册相应套接字读写事件和相应的回调函数hanlder 处理浏览器请求的信息读写 完成session过程
    # 当事件发生时候 相应的套接字会调用相应的回调函数， epoll 比select  更优
    # 1.因为 减少 用户态和内核态copy
    # 2.基于事件回调，直接回调函数 训练次数减少，创建次数减少
    def add_handler(
        ...
        # 设置读事件和回调 其实回调就是创建connection 和stream 进行请求 头部 和body 的读取
        if events & IOLoop.READ:
            self.asyncio_loop.add_reader(fd, self._handle_events, fd, IOLoop.READ)
            self.readers.add(fd)
        # 设置写事件 和回调，hanlder 处理完请求将 数据写入套接字后 触发回调 http 返回给浏览器
...
class AsyncIOMainLoop(BaseAsyncIOLoop):
    ...
    """
    '''
    ioloop 子孙类，实现 创建loop 其实际是有其父类 BaseAsyncIOLoop 完成创建loop 
    '''
